        ...
        <div className="p-4 border-b border-[var(--color-court-border)]">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-[var(--color-court-dark)] border border-[var(--color-court-border)] rounded-lg pl-12 pr-3 py-3 text-sm text-white focus:outline-none focus:border-[var(--color-gold-primary)]"
            />
          </div>
          ...
          {filteredFriends.length === 0 ? (
            ...
            filteredFriends.map(friend => (
              <div
                ...
                className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all ${selectedIds.has(friend.id)
                    ? 'bg-[var(--color-gold-dark)]/20 border border-[var(--color-gold-dark)]'
                    : 'hover:bg-[var(--color-court-dark)] border border-transparent'
                  }`}
              >
                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${selectedIds.has(friend.id)
